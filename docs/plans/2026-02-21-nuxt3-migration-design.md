## Nuxt 3 Migration Design for kfzgutachten-karakale.de

### Objectives

- Move from Nuxt 2.17 + Vue 2.7 to Nuxt 3 (or 4 when stable) + Vue 3 while:
  - Preserving current routes, SEO meta, and sitemap behavior.
  - Keeping static hosting (SSG) as the primary deployment mode.
  - Reducing JS bundle size and leveraging modern performance features (Vite, Nitro, better tree-shaking, delayed hydration).[3](https://nuxt.com/blog/v3-16)[4](https://www.pagespeedfix.com/blog/nextjs-vs-nuxt-performance/)[5](https://nuxt.com/docs/3.x/guide/concepts/vuejs-development)[1](https://masteringnuxt.com/blog/nuxt-4-performance-optimization-complete-guide-to-faster-apps-in-2026)

### Target Project Structure

- Root-level Nuxt 3 app (for example):
  - `app/` or `src/` (depending on chosen Nuxt 3 convention)
  - `pages/`
...
- Routing:
  - Nuxt 3 file-based routing preserves the same paths as today (`/`, `/about`, `/kontakt`, etc.) so existing links and SEO remain valid.

### Dependency Strategy

Replace Vue 2–only libraries with Vue 3–ready equivalents or lighter alternatives:

- **bootstrap-vue (Vue 2, bootstrap 4)**:
  - Replace with:
...
- **vue2-google-maps**:
  - The current Nuxt 2 app no longer uses this plugin; for Nuxt 3:
...
- **vue-awesome-swiper (Swiper 5)**:
  - Upgrade to Swiper 9+ or 10+ (which has better tree-shaking) and use the official Vue 3 Swiper components.
  - Keep hero/testimonial sliders but:
    - Lazy-load non-critical sliders (e.g. testimonials) using dynamic imports and `<ClientOnly>` in Nuxt 3.
...
- **Removed in Nuxt 2 and not needed in Nuxt 3**:
...
### Nuxt 3 Configuration Mapping

- **App configuration**:
  - Map `nuxt.config.js` settings to `nuxt.config.ts`:
    - `target: 'static'` → use `ssr: true` with `nitro` static preset or use `nuxi generate` for full SSG.
    - `head` → `app.head` in Nuxt 3:
...
- **Modules**:
  - `@nuxtjs/style-resources`:
    - Replace with Nuxt 3’s `css` + `vite.css.preprocessorOptions` or global SCSS in `nuxt.config.ts` (e.g. `css: ['@/assets/scss/style.scss']` and SCSS variables via `vite` options).
  - `@nuxtjs/redirect-module`:
    - Implement redirects using Nitro route rules or server middleware if still needed.
  - `@nuxtjs/sitemap`:
    - Use a Nuxt 3–compatible sitemap module or Nitro route introspection to generate `sitemap.xml`. Keep the explicit routes and `changefreq` similar to the current configuration.

- **Build/Performance**:
  - Use Vite (default in Nuxt 3) for:
    - Faster dev HMR.
  ...
  - Enable modern features:
    - Lazy/delayed hydration for non-interactive sections (e.g. static hero text, content blocks) where possible, so the browser doesn’t hydrate everything immediately.

### Component Migration Approach

- **Pages**:
  - Port each existing page component (`pages/*.vue`) to Nuxt 3 with minimal logic changes:
    - Replace `head()` with Nuxt 3’s `useHead` composable in `<script setup>` where appropriate, or keep the options API and use `definePageMeta`/`head` in Nuxt 3.
...
- **Routing and navigation**:
  - Keep link structure the same (`<NuxtLink>` in Nuxt 3) and maintain navigation hierarchy (header, footer, mobile menu).

### Migration Phasing

- **Phase A – Create Nuxt 3 skeleton**:
  - Scaffold a new Nuxt 3 project in a separate directory or Git worktree.
...
- **Phase C – Replace legacy integrations**:
  ...
  - Implement consent banner and analytics tracking using Nuxt 3–friendly patterns.
...
This design gives you a clear mapping from the existing Nuxt 2 static site to a Nuxt 3 application, with specific dependency replacements and configuration targets but without forcing the migration to happen immediately. It’s ready to use as the blueprint when you start the actual port.
